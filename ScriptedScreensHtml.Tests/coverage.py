...
table(["Area", "Handled", "Of", "Share"], [
    ...
    ("CSS pseudo-classes and pseudo-elements", len(sel_have & (mdn_sel_norm | sel_have)), len(sel_have) + len(sel_miss), "%d%%" % (100 * len(sel_have) // (len(sel_have) + len(sel_miss)))),
...
for g in sorted(groups_all, key=lambda g: (-len([k for k in groups_all[g] if k not in css_props]), g)):
    ...
    rows.append((g, len(ks) - len(miss_g), len(ks), names(miss_g)))
table(["Group", "Handled", "Of", "Not handled"], rows)

section("CSS at-rules")
table(["Handled", "Not handled"], [(names("@" + a for a in at_have), names("@" + a for a in at_miss))])

section("CSS selectors")
table(["Kind", "Handled", "Not handled"], [
    ("pseudo-classes", names(sorted(k for k in sel_have if not k.startswith("::"))), names(k for k in sel_miss if not k.startswith("::"))),
    ("pseudo-elements", names(sorted(k for k in sel_have if k.startswith("::"))), names(k for k in sel_miss if k.startswith("::"))),
])

section("CSS functions and units")
table(["Kind", "Handled", "Not handled"], [
    ("functions", names(fn_have), names(fn_miss)),
    ("units", names(sorted(units)), names(unit_miss)),
])

section("HTML elements")
table(["Handled", "Not handled"], [(names(html_have), names(html_missing))])

section("JavaScript: DOM and Web APIs")
table(["Object", "Handled", "Of", "Not handled"], [(g, len(h), len(h) + len(m), names(m)) for g, (h, m) in js_report.items()])
