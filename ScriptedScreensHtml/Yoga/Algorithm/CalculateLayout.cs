...
namespace ScriptedScreensHtml.Yoga
{
    public static partial class LayoutAlgorithm
    {
            ...
            ref LayoutData layoutMarkerData, LayoutPassReason reason)
        {
            ...
            {
                if (Event.Enabled) Event.Publish(node, EventType.MeasureCallbackStart);

                var measuredSize = node.Measure(
                    innerWidth, widthSizingMode.ToMeasureMode(),
                    innerHeight, heightSizingMode.ToMeasureMode());

                if (Event.Enabled) Event.Publish(node, EventType.MeasureCallbackEnd,
        ...
        if (Event.Enabled) Event.Publish(node, EventType.NodeLayout,
        ...
        {
        if (Event.Enabled) Event.Publish(node, EventType.LayoutPassStart);
        ...
        if (Event.Enabled) Event.Publish(node, EventType.LayoutPassEnd,
            new Event.LayoutPassEndData { LayoutData = markerData });
        }
    }

    public sealed class ListSegmentEnumerator : IEnumerator<Node>
    {
