...
namespace ScriptedScreensHtml.Yoga
{
    public class Style
    {
        ...
        {
            return _pool.GetSize(_maxDimensions[(int)axis]);
        }

        public void SetMaxDimension(Dimension axis, StyleSizeLength value)
        {
            _pool.Store(ref _maxDimensions[(int)axis], value);
        }
        ...
        {
            FlexDirection flexDirectionForDimension = dimension == ScriptedScreensHtml.Yoga.Dimension.Width
                ? FlexDirection.Row
        ...
        {
            var gap = dimension == ScriptedScreensHtml.Yoga.Dimension.Width ? ComputeColumnGap() : ComputeRowGap();
            return Comparison.MaxOrDefined(gap.Resolve(ownerSize).UnwrapOrDefault(0.0f), 0.0f);
        }
        ...
        {
            if (_gap[(int)Gutter.Column].IsDefined)
            {
                return _pool.GetLength(_gap[(int)Gutter.Column]);
            }
            else
            {
                return _pool.GetLength(_gap[(int)Gutter.All]);
            }
        }

        private StyleLength ComputeRowGap()
        {
            if (_gap[(int)Gutter.Row].IsDefined)
            {
                return _pool.GetLength(_gap[(int)Gutter.Row]);
            }
            else
            {
                return _pool.GetLength(_gap[(int)Gutter.All]);
        ...
        private StyleLength ComputeLeftEdge(StyleValueHandle[] edges, Direction layoutDirection)
        {
            if (layoutDirection == Direction.LTR &&
                edges[(int)Edge.Start].IsDefined)
            {
                return _pool.GetLength(edges[(int)Edge.Start]);
            }
            else if (layoutDirection == Direction.RTL &&
                     edges[(int)Edge.End].IsDefined)
            {
                return _pool.GetLength(edges[(int)Edge.End]);
            }
            else if (edges[(int)Edge.Left].IsDefined)
            {
                return _pool.GetLength(edges[(int)Edge.Left]);
            }
            else if (edges[(int)Edge.Horizontal].IsDefined)
            {
                return _pool.GetLength(edges[(int)Edge.Horizontal]);
            }
            else
            {
                return _pool.GetLength(edges[(int)Edge.All]);
            }
        }
        ...
        private StyleLength ComputeTopEdge(StyleValueHandle[] edges)
        {
            ...
            {
                return _pool.GetLength(edges[(int)Edge.Top]);
            }
            else if (edges[(int)Edge.Vertical].IsDefined)
            {
                return _pool.GetLength(edges[(int)Edge.Vertical]);
            }
            else
            {
                return _pool.GetLength(edges[(int)Edge.All]);
        ...
        private StyleLength ComputeRightEdge(StyleValueHandle[] edges, Direction layoutDirection)
        {
            ...
            {
                return _pool.GetLength(edges[(int)Edge.Right]);
            }
            else if (edges[(int)Edge.Horizontal].IsDefined)
            {
                return _pool.GetLength(edges[(int)Edge.Horizontal]);
        ...
        private StyleLength ComputeBottomEdge(StyleValueHandle[] edges)
        {
            ...
            {
                return _pool.GetLength(edges[(int)Edge.Bottom]);
            }
            else if (edges[(int)Edge.Vertical].IsDefined)
            {
                return _pool.GetLength(edges[(int)Edge.Vertical]);
