...
| Element.innerHTML — the elements markup makes, after the write | ✅ | elements of the page like any other: `getElementById`, a listener attached after the write (dropped when markup is written again: `v_unlisten`), attribute reads. A list or first match over elements of only some shapes, and a write to an element markup makes (the next markup write would undo it), are refused for now | PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: "innerHTML choosing between two shapes, with listeners on the buttons each one makes"; PlainTranslatorTests refusals: "a list over markup that has more than one shape", "a write to an element markup makes" | |
...
| Element.innerHTML — a list repeated over an array (`.map(…).join('')`, a loop adding markup) | | innerHTML part 2 | PlainTranslatorTests refusals: "a list of markup (innerHTML part 2)", "markup built up in steps" | |
| Element.innerHTML — `+=` | | innerHTML part 2 | PlainTranslatorTests refusals: "markup added with += (innerHTML part 2)" | |
