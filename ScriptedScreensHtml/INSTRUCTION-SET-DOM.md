...
| Window.innerHeight | ✅ | a constant: the height the page is laid out at, in the console's own proportions, rounded to a whole CSS pixel where the layout size is made (`PlainTranslator.ConsoleLayout`): 460, not 459, when the console's world aspect arrives a hair short | PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: "innerWidth, innerHeight and devicePixelRatio: the console the page is compiled for"; PlainTranslatorTests: "plain size: a square console whose world aspect arrives a hair short" and "a tall console whose world aspect arrives a hair short"; PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: plain-globals.lua (the in-game page) | |
| Window.outerWidth | | | | |
| Window.outerHeight | | | | |
...
| Document.getElementById() | ✅ | resolved at compile time: a literal id is its element, and the Lua holds its number when the script keeps it as a value. An id from a fixed set (a constant, a ternary, a function's arguments at every call) or a fixed text around one value (`'row' + i`, `` `cell${i}` ``, the value from a loop counter's range or any value, matched against the page's ids) is looked up by that value in a table built once (`V_IDn[i]`, never an id built as a string); an operation on it picks the element's own code from a table (`V_Xn`). The id can come through a helper every call of which gives it (`const $ = (id) => document.getElementById(id)`), from a field of an object literal read through `this` in its getter (`get el() { return $(this.id); }`, the objects pushed into an array in a loop, `o.el` then an element wherever o is one of them), and name an element markup makes (an id the markup gives it, never one the build made up). An id the compile cannot bound is refused | PlainTranslatorTests: every case; PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: "an element chosen at run time: an id built from a loop counter, an array of elements, an element passed to a function"; PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: "an element found by an id built at run time: through a `$` helper, from a getter reading this.id of objects pushed in a loop, and each button markup makes found by 'pad' + i"; PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: plain-runtime-el.lua (the in-game page, not yet seen in game); PlainTranslatorTests refusals: "an element chosen by an id the compile cannot bound" | |
...
| Element.innerHTML (write; part 1: markup of a fixed shape into an element the script names) | ✅ | compiled once: the markup's string literals are structure and the values between them slots, so the Lua writes values and picks shapes and builds no string. Read from the source as a template - literals, `+`, template literals, a ternary between markup, a helper function returning markup (its parameters, constants, a parameter given a default, `if … return`), a constant holding markup - with each value in the scope it is written in. A helper returning part of a tag - one or more whole attributes, `' data-act="' + (acts.length - 1) + '"'` - is followed as markup too: the attribute's name is fixed and its value a hole, and a helper whose result is `''` in some calls (`g.pick ? act(g.pick) : ''`) is a choice between having the attribute and not; a statement it runs first (`acts.push(fn);`) is done in the Lua right before the write its value lands in, and a function it is handed that reads a list's row keeps that row in a local as it is made (`do local V_Kn = V_R1.a[0] … end`), as a closure does. More than 32 shapes in one write (the only write into its element, made as the page loads): each choice between elements a switch, every one of its shapes laid out in place and one shown, a state of its own (`v_state(V_SHn, k)`), laid out with another only where they move the same things or where one shows what is hidden at rest and the other moves what is outside it; refused naming the choices: one inside a tag, one of more than 32 shapes itself, one whose shapes share a list, and those that still take more than 160 combinations together. Refused by name: an element chosen at run time, `&&`/`||` choosing markup, a tag name that is a value, an attribute name only known at run time, a value in `id` or an `on…` attribute, two values in one text or attribute whose helpers do something first, a helper doing something with no value of its markup after it | PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: "innerHTML as rich text: a value in bold, a word shown by a ternary, a colour from a fixed set"; PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: "innerHTML of elements from a helper function: text, a class and a style width from values"; PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: "innerHTML choosing between two shapes, with listeners on the buttons each one makes"; PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: "innerHTML written by a timer, the list as the page wrote it shown until then"; PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: plain-markup.lua (the in-game page: a parameter's default in a style width, a shape not shown at rest); PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: "clicks kept as functions in an array: a helper pushes each and returns the attribute data-act=…" and plain-acts.lua (the in-game page); PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: "one write with more than 32 shapes: five badges that come and go, …" and "one write with more than 32 shapes whose choices sit in different places: …", plain-styles.lua (the in-game page); PlainTranslatorTests refusals: "markup chosen with &&", "markup whose tag is a value" | |
| Element.innerHTML (write; part 1: markup of a fixed shape into an element the script names) | ✅ | compiled once: the markup's string literals are structure and the values between them slots, so the Lua writes values and picks shapes and builds no string. Read from the source as a template - literals, `+`, template literals, a ternary between markup, a helper function returning markup (its parameters, constants, a parameter given a default, `if … return`), a constant holding markup - with each value in the scope it is written in. A helper returning part of a tag - one or more whole attributes, `' data-act="' + (acts.length - 1) + '"'` - is followed as markup too: the attribute's name is fixed and its value a hole, and a helper whose result is `''` in some calls (`g.pick ? act(g.pick) : ''`) is a choice between having the attribute and not; a statement it runs first (`acts.push(fn);`) is done in the Lua right before the write its value lands in, and a function it is handed that reads a list's row keeps that row in a local as it is made (`do local V_Kn = V_R1.a[0] … end`), as a closure does. Refused by name: more than 32 shapes, `&&`/`||` choosing markup, a tag name that is a value, an attribute name only known at run time, a value in `id` or an `on…` attribute, two values in one text or attribute whose helpers do something first, a helper doing something with no value of its markup after it | PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: "innerHTML as rich text: a value in bold, a word shown by a ternary, a colour from a fixed set"; PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: "innerHTML of elements from a helper function: text, a class and a style width from values"; PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: "innerHTML choosing between two shapes, with listeners on the buttons each one makes"; PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: "innerHTML written by a timer, the list as the page wrote it shown until then"; PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: plain-markup.lua (the in-game page: a parameter's default in a style width, a shape not shown at rest); PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: "clicks kept as functions in an array: a helper pushes each and returns the attribute data-act=…" and plain-acts.lua (the in-game page); PlainTranslatorTests refusals: "markup chosen with &&", "markup whose tag is a value" | |
...
| Element.innerHTML — into an element chosen at run time | ✅ | the markup laid out once into each element the write can reach (a lookup by an id from a fixed set, an element of a list, an element passed to a function, an id a helper is given), each its own shapes, lists and states as if the script named it; the Lua picks the element's write by the number it holds (`local V_el = …` then `if V_el == n then … elseif …`), and none of them is the browser's TypeError on null (`v_nullref`). An element a write at load does not reach shows what the page wrote there until one does. Refused by name: an id the compile cannot bound, an element picked by an expression that does more than look it up, an id in the markup (each element would make one) | PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: "innerHTML written into an element chosen at run time: a lookup by a ternary id, and an element of a list passed to a function"; PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: "innerHTML written into an element found by an id a helper is given: a loop's rows, their count and colour from the call"; PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: plain-runtime-el.lua (the in-game page, not yet seen in game); PlainTranslatorTests refusals: "markup written into an element chosen by an id the compile cannot bound", "markup with an id written into an element chosen at run time" | |
...
| Element.getAttribute() | ✅ | an attribute the script changes (or looks up by a run-time name): the program's table of the element's attributes (`v_attr(at[name])`, the page's own values to start); any other, the constant its source gives, or null. The compile's own markers (a click region's `data-click` and the like) are not the page's: null, as for any attribute the page never wrote. `class` and `style` through it are refused for now | PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: "setAttribute, removeAttribute, toggleAttribute and dataset on attributes CSS selects on"; PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: "attributes nothing selects on: kept as the program's own values"; PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: "an attribute the page never wrote reads null on an element markup makes, a click listener on it and the compile's own click region with it" | |
...
| HTMLElement.hidden | ✅ | the browser's own `[hidden] { display: none }` as laid-out states, a user-agent rule that gives way to a `display` from the page's own CSS (`HtmlRenderer.HiddenByAttribute`): the element keeps its shapes in the scene inside `G v=$name_v` (0: not drawn and not clickable, as display: none), placed where each layout would put it shown, and what follows moves up as the browser moves it (`v_hidden`). Read: `(at.hidden ~= nil)`. Two elements whose hiding moves the same box (two toggled panels above one list) are refused for now, as two elements' classes are | PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: "hidden: the element leaves the layout and comes back, and one hidden from the start shows"; PlainTranslatorTests, each at 460x460, 1036x460 and 460x1036: "hidden under a display rule of the page's own: the rule wins, as it does over a browser's [hidden] { display: none }" | |
| HTMLElement.inert | | | | |
...
**1,715 total data rows** (verified by counting table rows in the assembled file — the subtotals below are approximate hand counts, kept for orientation): 4.1-4.4 (Window/globals ~212, Document/Node/ParentNode/ChildNode/Element/HTMLElement ~366, per-element HTML interfaces ~488, CharacterData/Text/CSSStyleDeclaration/DOMTokenList/DOMStringMap/NamedNodeMap/Attr ~54) plus 4.5 (events ~196, canvas ~92, SVG ~102, Web Animations ~34, CSSOM View 5, observers ~40, URL/text/binary ~71, grouped long tail 58 — after removing 22 rows duplicated against 4.1/4.2/4.3 during assembly).
