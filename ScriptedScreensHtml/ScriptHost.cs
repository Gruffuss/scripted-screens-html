...
internal sealed class ScriptHost : IDisposable
{
    ...
    public bool Frame(float nowSeconds, Dictionary<string, VisualElement> elements)
    {
        ...
        if (_busy)
        {
            // a read on the worker waits for its writes: apply them now so the read is right, and
            // emit once, when the frame is done, rather than once per read (a render's getAttribute
            // loop was an emit per element)
            if (_syncing && Pump()) _pumpedMidFrame = true;
            return false;
        }
        var any = Pump() | _pumpedMidFrame;
        _pumpedMidFrame = false;
        return any;
    }
