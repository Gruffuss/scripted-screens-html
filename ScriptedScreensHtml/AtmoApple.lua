-- AtmoApple.lua -- the Apple-style Atmo Regulator mockup (AtmoUi/ios) as a page: AtmoApple.html verbatim, embedded.
-- Generated from AtmoApple.html by the snippet in PLAN.md; edit the html, not this file. Push to the 3x3 console (586).
...
function renderConfig() {
  ...
  const bindMask = 'linear-gradient(' + (st.bindAtStart === false ? 'rgba(0,0,0,.2) 0,#000 8%,' : '#000 0,') + (st.bindAtEnd ? '#000 100%)' : '#000 92%,rgba(0,0,0,.2) 100%)');
...
function render(structural) {
  ...
  if (list) list.addEventListener('scroll', () => {
    ...
    if (end !== !!st.bindAtEnd || start !== (st.bindAtStart !== false)) { st.bindAtEnd = end; st.bindAtStart = start; render(true); }
  });
  lastTab = st.tab;
}
applyTheme();
